...
import { arrayBufferToBase64, base64ToString } from './base64'
...
export class GitHubSync {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        if (res.status === 422) {
          ...
          throw new Error(err?.message || '422')
        }
        const err: any = res.json
        lastErr = new Error(`GitHub API ${res.status}: ${err?.message || 'unknown'}`)
      } catch (e: any) {
        if (e.message === '404' || e.message === '409: conflict') throw e
        lastErr = e
      }
  ...
  async loadState(): Promise<void> {
    try {
      ...
      this.state = {
        lastRemoteSHA: parsed.lastRemoteSHA || '',
        files: parsed.files || {},
      }
    } catch {
  ...
  async restoreBackup(): Promise<void> {
    try {
      ...
      // backup não existe — nada a restaurar
    }
    await this.ensureDataDir()
  ...
  async status(): Promise<{
    ...
    for await (const f of this.walkFiles('/')) {
      ...
      try {
        stat = await this.vault.adapter.stat(f)
      } catch {
        continue
      }
  ...
  async quickStatus(): Promise<{ remoteAhead: boolean; branch: string }> {
    try {
      ...
      return { remoteAhead: false, branch: 'main' }
    }
  }
  ...
  async push(onPhase?: (msg: string) => void): Promise<string[]> {
    ...
    for await (const f of this.walkFiles('/')) {
      ...
      const hash = await sha256(buf)
      if (!cached || cached.sha !== hash) {
        changed.push({ path: f, hash, size: stat.size })
      }
    }
    ...
    if (skipped.length > 0) {
      log(`pulado ${skipped.length} arquivos > 1MB`)
    }
    ...
    for (const path of deleted) {
      try {
        let remote: any
        try {
          remote = await this.apiGet(`/contents/${path}?ref=main`)
        } catch {
